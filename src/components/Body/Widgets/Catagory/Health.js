...
function Health() {
  ...
  const url = 'https://globapi.herokuapp.com/onenews?category=health&country=in'
  useEffect(() => {
      ...
      <div className="articlist">
        {
          Data.map((elm, idx) => {
            if (idx <=6 && idx!=0)
              return <Listarticle tag={'Health'} title={elm.title} date={elm.publishedAt.slice(0, 10)} image={elm.urlToImage}  url={elm.url}/>
          })
        }
      </div>
    </div>
